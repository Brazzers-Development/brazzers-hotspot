...
RegisterNetEvent('brazzers-hotspot:client:createPickupZone', function()
    ...
    hotspotPickupZone:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            TriggerServerEvent('brazzers-hotspot:server:getItem')
            hotspotPickupZone:destroy()
        end
    end)
end)

RegisterNetEvent('brazzers-hotspot:client:removeHotspotBlip', function()
    RemoveBlip(blip)
end)
