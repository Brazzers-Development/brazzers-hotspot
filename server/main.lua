...
RegisterNetEvent('brazzers-hotspot:server:purchaseMilkroadItems', function(data)
    ...
    TriggerClientEvent('brazzers-hotspot:client:createPickupZone', src)
end)

RegisterNetEvent('brazzers-hotspot:server:getItem', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    removeFromCart(cid)
    TriggerClientEvent('brazzers-hotspot:client:removeHotspotBlip', src)
end)